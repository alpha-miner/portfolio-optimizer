...
TEST(LpOptimizerTest, SimpleCaseWithInterior) {
    constexpr size_t n = 10;
    double consMat[n+2];
    double lower[n];
    double upper[n];
    ...
    LpOptimizer optimizer(n, 1, consMat, lower, upper, objective, "interior");
    ...
    for(size_t i=0; i != n; ++i) {
        if(i < 5)
            ASSERT_NEAR(x[i], 0.2, 1e-6);
        else
            ASSERT_NEAR(x[i], 0., 1e-6);
    }

    ASSERT_NEAR(f_val, 2., 1e-6);
}